...
public class ExceptionControlUnit {
    public static ExceptionControlUnitOutputParameters compute(ExceptionControlUnitInputParameters parameters) {
        ...
        if (parameters.exc_trig_sp()) {
            triggeredExceptions.add(Processor.ExceptionNumbers.UNALIGNED_SP);
        }
        if (parameters.exc_trig_pc()) {
            triggeredExceptions.add(Processor.ExceptionNumbers.UNALIGNED_PC);
        }
        if (parameters.exc_trig_invalid_inst()) {
            triggeredExceptions.add(Processor.ExceptionNumbers.INVALID_INST);
        }
        if (parameters.exc_trig_ext()) {
            triggeredExceptions.add(Processor.ExceptionNumbers.EXTERNAL_EXC);
        }
