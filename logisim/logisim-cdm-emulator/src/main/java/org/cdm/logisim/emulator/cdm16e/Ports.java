package org.cdm.logisim.emulator.cdm16e;

public class Ports {
    public static final int DATA_IN = 0;
    public static final int DATA_OUT = 1;
    public static final int ADDRESS = 2;
    public static final int MEM = 3;
    public static final int DATA = 4;
    public static final int READ = 5;
    public static final int WORD = 6;
    public static final int IRQ = 7;
    public static final int INT_NUMBER = 8;
    public static final int EXC = 9;
    public static final int CLK = 10;
    public static final int EXC_NUMBER = 11;
    public static final int HOLD = 12;
    public static final int STATUS = 13;
    public static final int R0 = 14;
    public static final int R1 = 15;
    public static final int R2 = 16;
    public static final int R3 = 17;
    public static final int R4 = 18;
    public static final int R5 = 19;
    public static final int R6 = 20;
    public static final int FP = 21;
    public static final int PC = 22;
    public static final int SP = 23;
    public static final int PS = 24;
    public static final int FETCH = 25;
    public static final int IAck = 26;
    public static final int IO_HEADER = 27;
    public static final int VEC = 28;
    public static final int CTX_NUMBER = 29;
}
