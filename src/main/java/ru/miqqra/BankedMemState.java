...
class BankedMemState implements InstanceData, Cloneable, HexModelListener {
    ...
    @Override
    public BankedMemState clone() {
        try {
            BankedMemState ret = (BankedMemState) super.clone();
    ...
    private void setBits(int addrBits, int dataBits) {
        if (contents == null) {
            contents = BankedMemContents.create(addrBits, dataBits);
        } else {
    ...
    public BankedMemContents getContents() {
        return contents;
    }
