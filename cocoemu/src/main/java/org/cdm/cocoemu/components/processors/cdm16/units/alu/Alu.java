...
public class Alu {
    ...
    public static AluOutputParameters compute(AluInputParameters parameters) {
        ...
        int cIn = Arithmetic.toInteger(parameters.cIn());
        ...
        switch (parameters.alu_op_type()) {
            case Cdm16.ALU_InstructionGroups.ALU_3:
                switch (parameters.alu_func()) {
                    case Cdm16.ALU_3op.AND:
                        rd = rs0 & rs1;
                        break;
                    case Cdm16.ALU_3op.OR:
                        rd = rs0 | rs1;
                        break;
                    case Cdm16.ALU_3op.XOR:
                        rd = rs0 ^ rs1;
                        break;
                    case Cdm16.ALU_3op.BIC:
                        rd = rs0 & (~rs1);
                        break;
                    case Cdm16.ALU_3op.ADD:
                        rd = rs0 + rs1;
                        cOut = checkC(rd);
                        vOut = checkV(Arithmetic.normalize(rd), rs0, rs1);
                        break;
                    case Cdm16.ALU_3op.ADC:
                        rd = rs0 + rs1 + cIn;
                        cOut = checkC(rd);
                        vOut = checkV(Arithmetic.normalize(rd), rs0, rs1 + cIn);
                        break;
                    case Cdm16.ALU_3op.SUB:
                        rd = rs0 + (rs1 ^ 0xFFFF) + 1;
                        cOut = checkC(rd);
                        vOut = checkV(Arithmetic.normalize(rd), rs0, (rs1 ^ 0xFFFF) + 1);
                        break;
                    case Cdm16.ALU_3op.SBC:
                        rd = rs0 + (rs1 ^ 0xFFFF) + cIn;
                        cOut = checkC(rd);
                        vOut = checkV(Arithmetic.normalize(rd), rs0, (rs1 ^ 0xFFFF) + cIn);
                        break;
                }
                break;
            case Cdm16.ALU_InstructionGroups.ALU_2:
                switch (parameters.alu_func()) {
                    case Cdm16.ALU_2op.NEG:
                        rd = (rs0 ^ 0xFFFF) + 1;
                        cOut = checkC(rd);
                        vOut = Arithmetic.toInteger(rs0 == 0x8000);
                        break;
                    case Cdm16.ALU_2op.NOT:
                        rd = ~rs0;
                        break;
                    case Cdm16.ALU_2op.SXT:
                        rd = Arithmetic.signExtend(rs0);
                        break;
                    case Cdm16.ALU_2op.SCL:
                        rd = rs0 & 0x00FF;
                        break;
                }
                break;
            case Cdm16.ALU_InstructionGroups.SHIFTS:
                int shiftCount = parameters.shift_count_d();

                shiftCount++;

                switch (parameters.alu_func()) {
                    case Cdm16.ALU_Shifts.SHL:
                        rd = rs0 << shiftCount;
                        cOut = Arithmetic.toInteger(Arithmetic.testBit(rs0, 16 - shiftCount));
                        break;
                    case Cdm16.ALU_Shifts.SHR:
                        rd = rs0 >>> shiftCount;
                        cOut = Arithmetic.toInteger(Arithmetic.testBit(rs0, shiftCount - 1));
                        break;
                    case Cdm16.ALU_Shifts.SHRA:
                        int sign = checkN(rs0);
                        int mask = Arithmetic.toBoolean(sign) ? -1 : 0;
                        rd = (rs0 >> shiftCount) | (mask << 16 - shiftCount);
                        cOut = Arithmetic.toInteger(Arithmetic.testBit(rs0, shiftCount - 1));
                        break;
                    case Cdm16.ALU_Shifts.ROL:
                        rd = (rs0 << shiftCount) | (rs0 >> (16 - shiftCount));
                        cOut = Arithmetic.toInteger(Arithmetic.testBit(rs0, 16 - shiftCount));
                        break;
                    case Cdm16.ALU_Shifts.ROR:
                        rd = (rs0 >> shiftCount) | (rs0 << (16 - shiftCount));
                        cOut = Arithmetic.toInteger(Arithmetic.testBit(rs0, shiftCount - 1));
                        break;
                    case Cdm16.ALU_Shifts.RCL:
                        rd = (rs0 << shiftCount) | (cIn << shiftCount - 1) | (rs0 >> (16 - shiftCount + 1));
                        cOut = Arithmetic.toInteger(Arithmetic.testBit(rs0, 16 - shiftCount));
                        break;
                    case Cdm16.ALU_Shifts.RCR:
    ...
    private static int checkN(int value) {
        return Arithmetic.toInteger(Arithmetic.testBit(value, 15));
    }

    private static int checkC(int value) {
        return Arithmetic.toInteger(Arithmetic.testBit(value, 16));
    }
