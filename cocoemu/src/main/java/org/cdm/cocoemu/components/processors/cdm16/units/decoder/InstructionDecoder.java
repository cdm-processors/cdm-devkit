...
import static org.cdm.cocoemu.components.processors.cdm16.units.BranchUnit.checkFlags;
...
public class InstructionDecoder {
    public static InstructionDecoderOutputParameters compute(InstructionDecoderInputParameters parameters) {
        ...
        int alu_op_type = Cdm16.ALU_InstructionGroups.ALU_3;

        int imm_type = Cdm16.IMM_Type.IMM_9;
        ...
        switch (inst_group) {
            case 0b000:
                if (X) {
                    microcode_address = Cdm16.Instructions.SHIFTS;
                    alu_op_type = Cdm16.ALU_InstructionGroups.SHIFTS;
                    switch_alu_func0 = true;
                } else {
                    if (Y) {
                        if (checkFlags(br_abs_flags_d, parameters.psFlags())) {
                            microcode_address = Cdm16.Instructions.BR_ABS;
                        } else {
                            microcode_address = Cdm16.Instructions.BR_ABS_NOP;
                        }
                    } else {
                        microcode_address = Cdm16.InstructionGroups.ZERO_OP + op_type_d0;
                    }
                }
                break;

            case 0b001:
                microcode_address = Cdm16.InstructionGroups.ONE_OP + op_type_d1;
                rs1 = rd_d;
            ...
            case 0b010:
                if (X) {
                    if (Y) {
                        microcode_address = Cdm16.Instructions.ALU_2;
                        alu_op_type = Cdm16.ALU_InstructionGroups.ALU_2;
                        switch_alu_func1 = true;
                    } else {
                        microcode_address = Cdm16.InstructionGroups.MEM_2 + op_type_d2;
                    }
                } else {
                    if (Y) {
                        microcode_address = Cdm16.Instructions.ALU_3_IND;
                        alu_op_type = Cdm16.ALU_InstructionGroups.ALU_3;
                        rs1 = rd_d;
                        switch_alu_func1 = true;
                    } else {
                        microcode_address = Cdm16.InstructionGroups.TWO_OP + op_type_d2;
                    }
                }
                break;

            case 0b011:
                microcode_address = Cdm16.InstructionGroups.IMM_6 + op_type_d3;
                rs0 = rd_d;
                imm_type = Cdm16.IMM_Type.IMM_6;

                break;

            case 0b100:
                microcode_address = Cdm16.InstructionGroups.IMM_9 + op_type_d3;
                break;

            case 0b101:
                if (X) {
                    microcode_address = Cdm16.Instructions.ALU_3;
                    alu_op_type = Cdm16.ALU_InstructionGroups.ALU_3;
                    switch_alu_func0 = true;
                } else {
                    microcode_address = Cdm16.InstructionGroups.MEM_3 + op_type_d3;
                }
                break;

            case 0b110:
                if (checkFlags(br_rel_flags_d, parameters.psFlags())) {
                    microcode_address = Cdm16.Instructions.BR_REL_N;
                } else {
                    microcode_address = Cdm16.Instructions.BR_REL_NOP;
                    br_rel_nop = true;
                }
                break;
            ...
            case 0b111:
                if (checkFlags(br_rel_flags_d, parameters.psFlags())) {
                    microcode_address = Cdm16.Instructions.BR_REL_P;
                } else {
                    microcode_address = Cdm16.Instructions.BR_REL_NOP;
                    br_rel_nop = true;
                }
        ...
        if (switch_alu_func0 || switch_alu_func1) {
            ...
            if ((microcode_address & 0b1110000) == Cdm16.InstructionGroups.IMM_6 && op_type_d3 >= 0b1110) {
                alu_func = Cdm16.ALU_3op.SUB;
            } else {
                alu_func = Cdm16.ALU_3op.ADC;
            }
        }
        ...
        if (microcode_address == Cdm16.InstructionGroups.IMM_9
                || microcode_address == Cdm16.InstructionGroups.IMM_9 + 1) {
            intInstruction = true;
        }
    ...
    public static InstructionDecoderOutputParameters getFetchSignals() {
        return new InstructionDecoderOutputParameters(
                Cdm16.Instructions.FETCH,
                0,
                0,
                0,
                0,
                Cdm16.ALU_InstructionGroups.ALU_3,
                false,
                false,
                Cdm16.ALU_3op.ADC,
                Cdm16.IMM_Type.IMM_6,
                0,
                false,
                false,
                false,
